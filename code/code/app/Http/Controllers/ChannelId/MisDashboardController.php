...
class MisDashboardController extends Controller
{
    ...
    public function misoaodashboard(Request $request)
    {
        ...
        //returns tempalte
        return view('channelid.misoaodashboard')->with('accountsCount',$accountsCount)->with('customerNames',$customerNames)
    ...
    public function oaoapplications(Request $request)
    {
        try {
            ...
            $filteredColumns = ['CREATED_DATE', 'USER_NAME','OAO_STATUS.PA_MISMATCH','OAO_STATUS.AOF_NUMBER','MOBILE_NUMBER','OAO_STATUS.QUERY_ID','OAO_STATUS.DEDUPE_STATUS','OAO_STATUS.PAYMENT','OAO_STATUS.FUND_RECEIVED','OAO_STATUS.CUSTOMER_NUMBER', 'OAO_STATUS.ACCOUNT_NUMBER','OAO_STATUS.FTR','OAO_STATUS.VKYC_LINK','OAO_STATUS.VKYC_STATUS', 'OAO_STATUS.FORM_ID','OAO_STATUS.PAN_NAME','OAO_STATUS.UPI_NAME','OAO_STATUS.UA_MISMATCH', 'OAO_STATUS.CUSTOMER_NUMBER', 'OAO_STATUS.ACCOUNT_NUMBER', 'OAO_STATUS.CUSTOMER_ID','OAO_STATUS.ACCOUNT_ID', 'OAO_STATUS.OAO_ID'];
            $i=0;
            //build dt array
            foreach ($filteredColumns as $column) {
                if($column == 'CREATED_DATE'){
                    array_push($array_column,strtolower('OAO_STATUS.DSA_CREATED_AT'));
                    $dt[$i] = array('db' => strtolower('OAO_STATUS.DSA_CREATED_AT'), 'dt' => $i, 'formatter' => function($d, $row){
                        $html = $row->dsa_created_at;
                        return $html;
                    });
                }
                else if($column == "USER_NAME")
                {
                    array_push($array_column,strtolower('OAO_STATUS.CUSTOMER_ID'));
                     $dt[$i] = array( 'db' => strtolower('OAO_STATUS.CUSTOMER_ID'),'dt' => $i,
                    ...
                    $dt[$i] = array( 'db' => strtolower('OAO_STATUS.PA_MISMATCH'),'dt' => $i,
                        'formatter' => function( $d, $row ) {
                            $html = $row->pa_mismatch.'  |  '.$row->ua_mismatch;
                            return $html;
                        }
                    );
                ...
                }else if($column == "CUSTOMER_NUMBER"){
                    array_push($array_column,strtolower('OAO_STATUS.CUSTOMER_NUMBER'));
                    $dt[$i] = array('db' => strtolower('OAO_STATUS.CUSTOMER_NUMBER'), 'dt' => $i, 'formatter' => function($d, $row){
                        $html = '';
                        $html = $row->customer_number;
                        return $html;
                    });
                }else if($column == "ACCOUNT_NUMBER"){
                    array_push($array_column,strtolower('OAO_STATUS.ACCOUNT_NUMBER'));
                    $dt[$i] = array('db' => strtolower('OAO_STATUS.ACCOUNT_NUMBER'), 'dt' => $i, 'formatter' => function($d, $row){
                        $html = '';
                        $html = $row->account_number;
                        return $html;
                    });
                    ...
                    $dt[$i] = array( 'db' => strtolower('OAO_STATUS.VKYC_LINK'),'dt' => $i,
                        'formatter' => function( $d, $row ) {
                            $html = '';
                            if($row->vkyc_link != ''){
                                $html = '<a href="javascript:void(0)" class="vkyc_link linktext badge badge-secondary" title="'.$row->vkyc_link.'">Y</a>';   
                            }
                            return $html;
                        }
                    );
            ...
            if( (isset($requestData['startDate']) && $requestData['startDate'] != '') && 
                    (isset($requestData['endDate']) && $requestData['endDate'] != '')
            ){
                $requestData['startDate'] = explode("-", $requestData['startDate']);
                $requestData['startDate'] = $requestData['startDate']['2'] . '-' . $requestData['startDate'][1] . '-'
                                                . $requestData['startDate'][0];
                 $requestData['endDate'] = explode("-", $requestData['endDate']);
                $requestData['endDate'] = $requestData['endDate']['2'] . '-' . $requestData['endDate'][1] . '-'
                                                . $requestData['endDate'][0];
                $requestData['startDate'] = Carbon::parse($requestData['startDate']);
                $requestData['endDate'] = Carbon::parse($requestData['endDate']);
                if($requestData['endDate']->diffInDays($requestData['startDate']) < 3){
                    $requestData['endDate'] = $requestData['endDate']->addDays(3);
                }
                $requestData['startDate'] = $requestData['startDate']->toDateString() . " 00:00:00";
                $requestData['endDate'] = $requestData['endDate']->toDateString() . " 23:59:59";
                $dt_obj = $dt_obj->whereRaw("DSA_CREATED_AT BETWEEN '" . $requestData['startDate'] ."' AND '". $requestData['endDate'] ."'" );
            }
